...
export default function LeadDashboard() {
  ...
  const myTeam = users.filter(u => u.role === 'team_member');
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: 960, margin: '0 auto' }}>
      <div style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 500, margin: 0, color: 'var(--color-text-primary)' }}>Lead Dashboard</h1>
          <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', margin: '4px 0 0' }}>
            Welcome, {user?.firstName}. Track your team's progress.
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: '1.5rem' }}>
        <StatCard label="My Submissions"   value={stats?.total}   color="blue"   />
        <StatCard label="Today's Entries"  value={stats?.today}   color="green"  />
        <StatCard label="Team Members"     value={myTeam.length}  color="purple" />
        <StatCard label="Active Projects"  value={projects.filter(p => p.isActive).length} color="amber" />
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        <Section title="My team members">
          {myTeam.length ? myTeam.slice(0, 6).map(u => (
            <div key={u._id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontSize: 13, color: 'var(--color-text-primary)' }}>{u.firstName} {u.lastName}</span>
              </div>
              <span style={{
          ...
          )) : <p style={{ fontSize: 13, color: 'var(--color-text-tertiary)' }}>No team members yet.</p>}
        </Section>
        ...
        <Section title="Top contributors">
          {stats?.byUser?.length ? stats.byUser.slice(0, 5).map((u, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 500, background: '#E6F1FB', color: '#0C447C', padding: '2px 10px', borderRadius: 20 }}>{u.count} entries</span>
            </div>
      ...
      <Section title="Assigned projects">
        <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
          ...
          <tbody>
            ...
            {!projects.length && <tr><td colSpan={3} style={{ padding: '1rem 0', color: 'var(--color-text-tertiary)', fontSize: 13 }}>No projects assigned.</td></tr>}
          </tbody>
        </table>
      </Section>
    </div>
